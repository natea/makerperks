...
- [x] 1.1 In `src/layouts/Base.astro`, change the Contribute CTA `href` from the org
...
- [x] 2.1 In `CONTRIBUTING.md`, update the `audience` step to state the accuracy rule
...
- [x] 3.1 Build; the Contribute link in the rendered header points to the
  `CONTRIBUTING.md` URL and the guide covers add/edit + the issue-form path
- [x] 3.2 Confirm the target URL resolves (no 404) for the actual repo slug
- [x] 3.3 `openspec validate add-contributing-guide --strict` passes; typecheck, lint,
  build green
